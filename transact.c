...
void ARCH_DEP(process_tbegin)(REGS *hregs, S16 i2, VADR effective_addr1)
{
  ...
  hregs->psw.cc = 0;        /* set cc=0 to indicate tranaction start */
  if (hregs->tranlvl == 1)  /* if starting the transaction  */
  {
    pmap = hregs->tpagemap;
    for (i = 0; i < MAX_TRAN_PAGES; i++, pmap++)
    {
    ...
    if (hregs->psw.states & 0x01)    /* problem state */
      hregs->tdbaddr = (TDB *)effective_addr1;     /* save the tcb address */
    else
      if (hregs->CR(2) & 0x04)       /* tdb only in problem state */
        hregs->tdbaddr = 0;
      else
    ...
    hregs->tranprogfiltlvl = i2 & 0x0003;  /* set initial filter level */
    ctlmask1 = hregs->CR(2) & 0x03;  /* isolate last two bits */
    switch (ctlmask1)               /* see what to do */
    {
    case 0:                          /* normal operation, do nothing */
    case 3:                          /* reserved, do nothing         */
      break;
    case 1:                          /* abort always                 */
...
/* End a transaction (constrained or unconstrained)                  */
/*-------------------------------------------------------------------*/
...
BYTE    mask = 0x00;
    ...
    hregs->psw.cc = 0;         /* clear the condition code   */
    ...
    case 1:                          /* abort always                 */
      rand1 = (U16)rand() % 10;
      hregs->tranabortnum = (U16)(rand() % 32);
      hregs->rabortcode = racode[rand1];
      break;
    case 2:                            /* abort randomly */
      rand1 = (U16)rand();             /* get a random number    */
      rand2 = (U16)rand();
