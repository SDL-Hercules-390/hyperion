...
TEST1    EQU   *
         ...
         LCTL  6,6,C6ZERO              Initialize CR6 - all bits off
...
*        * CR6 = X'00000004'  (CR6 bit 1=0 (virtual supervisor state), bit 29=1 (370E enabled))
********** No program checks should occur
*
TEST2    EQU   *
         LA    R8,2                    Test 2 in progress
         LCTL  6,6,C6ALLOW             Initialize CR6 - allow prob state execution
...
*        * CR6 = X'40000004'  (CR6 bit 1=1 (virtual PROBLEM state), bit 29=1 (370E enabled))
********** Program check 02 should occur
*
TEST3    EQU   *
         SVC   0                       Back to supervisor state
         LA    R8,3                    Test 3 in progress
         LCTL  6,6,C6VPROB             Initialize CR6 - virtual prob state + 370E
...
*
SUCCESS3 EQU   *
         MVC   RESULTS3,GOODRC         Set good result (expected program check occurred)
         B     TEST4                   Go to next test
*
FAIL3    EQU   *
         MVC   RESULTS3,BADRC          Test failed (E502 executed when it should not)
*
*
*
********** Execute Fix Page on simplified path return.
* TEST 4 * Real PSW in problem state
*        * CR6 = X'00000000'  (CR6 bit 1=0 (virtual Supv state), bit 29=0 (370E disabled))
********** Program check 02 should occur
*
TEST4    EQU   *
         SVC   0                       Back to supervisor state
         LA    R8,4                    Test 4 in progress
         LCTL  6,6,C6N370E             Initialize CR6 - virtual supv state + 370E OFF
...
*
SUCCESS4 EQU   *
         MVC   RESULTS4,GOODRC         Set good result (expected program check occurred)
         B     EOJ                     All tests completed
*
FAIL4    EQU   *
         MVC   RESULTS4,BADRC          Test failed (E502 executed when it should not)
...
PGMFLIH  EQU   *                       PGM Check Interruption Routine
         ...
         BE    HALT                    Halt test, program check not allowed
         C     R8,=F'2'                Doing test 2?		
         BE    HALT                    Halt test, program check not allowed
*
*                                      * tests 3&4 prog check 02 is expected
         CLI   PGMINTC+3,X'02'         priv-op exception?
         BNE   HALT                    No, something else wrong. Test # in R8.
         LPSW  PGMOPSW                 Resume execution, this is expected
*
HALT     LPSW  PGMHALT                 Halt on unexpected pgm check.
...
SVCFLIH  EQU   *                       SVC Interruption Routine
         ...
         BE    SVC001                  Yes
         LPSW  BADSVC                  Halt on bad SVC
*
SVC000   EQU   *
         NI    SVCOPSW+1,255-X'01'     Turn off problem state bit in old PSW
         LPSW  SVCOPSW                 Resume execution in supv state
*
SVC001   EQU   *
         OI    SVCOPSW+1,X'01'         Turn on problem state bit in old PSW
...
BADSVC   DC    X'000A000000000BAD'     Test end - Bad SVC #
PGMHALT  DC    X'000A00000000DEAD'     Unexpected program check
*  
*
         ORG   STRTLABL+X'600'
