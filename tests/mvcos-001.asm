...
***********************************************************************
*
* These tests and this programming were validated on a z114 using a
* z/VM 6.4 virtual machine on 27 January 2021.
*
***********************************************************************
*
...
* Literals are placed into a page frame beloning to each one of the
...
*                   Next loop 2
*                   Next loop 1
*                   Terminate with success, PSW=X'001'
*
...
*
SET000   EQU   *
         SSKE  R0,R1                   Set the key
         AHI   R1,4096                 Bump to next page
         BCT   R2,SET000
*
         LAM   0,15,AR0                Clear all ARs
         LCTLG 0,15,CR0                Load all the CRs
...
TOAS000  EQU   *
         ...
         BZ    TOAS200                 A is off, use PSW ASC
*
TOAS020  EQU   *                       A is on, do OAC1 AS changes
         LR    R1,R0                   Copy control bits
...
FRMAS000 EQU   *
         ...
         BZ    FRMAS200                A is off, use PSW ASC
*
FRMAS020 EQU   *                       A is on, do OAC2 AS changes
         LR    R1,R0                   Copy control bits
...
CHK100   EQU   *
         ...
         EAR   R2,1                    Get the ALET we loaded into AR1
...
*
*
SETAR    LAM   1,1,=F'0'    AS=00      Primary, set ALET=0
         CPYA  1,5             01      AR, set AR1 to MVCOS Operand 1
         LAM   1,1,=F'1'       10      Secondary, set ALET=1
         LAM   1,1,=F'2'       11      Home set ALET=2
*
GETALET  LA    2,0          AS=00      Primary, set ALET=0
         EAR   2,6             01      AR, set R2 to MVCOS Operand 2 AR
         LA    2,1             10      Secondary, set ALET=1
         LA    2,2             11      Home set ALET=2
...
GOODPSW  DC    X'0402400080000000',XL4'00',X'00000000' Success
HALT     DC    X'04024000',X'80000000',XL4'00',X'0000000F' Abnormal end
*
*-- Control registers
*
CR0      DC    X'00000000',X'04000000' Secondary space control bit 37
CR1      DC    X'00000000',A(SEGPRI)   Primary ASCE
CR2      DC    X'00000000',A(DUCT)     Dispatchable Unit Ctl Table
CR3      DC    X'00000000',X'C0000001' PKM=C000, Secondary ASN=1
CR4      DC    X'00000000',X'00000000' Primary ASN=0
CR5      DC    X'00000000',A(PASTEO)   Primary ASTE Origin
CR6      DC    X'00000000',X'00000000'
CR7      DC    X'00000000',A(SEGSEC)   Secondary ASCE
CR8      DC    X'00000000',X'00000000'
CR9      DC    X'00000000',X'00000000'
CR10     DC    X'00000000',X'00000000'
CR11     DC    X'00000000',X'00000000'
CR12     DC    X'00000000',X'00000000'
CR13     DC    X'00000000',A(SEGHOM)   Home ASCE
CR14     DC    X'00000000',X'00000000'
CR15     DC    X'00000000',X'00000000'
*
AR0      DC    16F'0'                  Init for access registers
...
*
CONTROL  DS    0F                      R0 MVCOS Control bits
OAC1     DS    H                        1st OAC
...
PASTEO   DS    0XL64                    Primary ASN Second Table Entry
         DC    A(0)           +0        ATO
         DC    A(0)            4        AX,ATL
         DC    A(0),A(SEGPRI)  8        Primary ASCE (same as CR1)
         DC    A(0)           16        ALD
         DC    A(0)           20        ASTESN
         DC    A(0)           24        LTD
...
*
*-- This DUCT is used by the primary space programming when in
*-- Access Register mode in order to use the DU-AL.
*
         ORG   STRTLABL+X'9100'
...
HASTEO   DS    0XL64                    Home ASN Second Table Entry
         DC    A(0)           +0        ATO
         DC    A(0)            4        AX,ATL
         DC    A(0),A(SEGHOM)  8        Home ASCE (same as CR13)
         DC    A(0)           16        ALD
         DC    A(0)           20        ASTESN
         DC    A(0)           24        LTD
         DC    A(0)           28        Ctl prog use
