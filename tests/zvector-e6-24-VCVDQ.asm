 TITLE 'zvector-e6-24-VCVDQ'
***********************************************************************
*
*        Zvector E6 instruction tests for VRI-j encoded:
*
...
*
***********************************************************************
*  This program tests proper functioning of the z/arch E6 VRI-j vector
*  convert to decimal (128), VCVDQ, instruction.
...
*   archlvl     z/Arch
*
*   loadcore    "$(testpath)/zvector-e6-24-VCVDQ.core" 0x0
*
...
*----------------------------------------------------------------------
CCCHECK  EQU   *
         MVC   CCFAILED,=CL1'N'       initializae cc failed
*
* extract CC from PSW
*
         L     R1,CCPSW
...
*
* check CC was expected
*
         CLC   CC,CCFOUND
         BNE   CCMSG

         BR    R15                    return from CCcheck

***********************************************************************
* cc was not as expected
***********************************************************************
CCMSG    EQU   *
         MVC   CCFAILED,=CL1'Y'        set cc failed
*
* FILL IN MESSAGE
*
...
*
         MVC   SKIPXC,=CL8'        '
         CLC   OPNAME,=CL8'VCVDQ   '
         BNE    0(R15)                  so exit
*
*        cross check depends on CC=3 overflow check
*
         CLC   CCFOUND,=XL1'3'
         BE    0(R15)                  so exit
*
*        cross check depends on CC check
*
         CLC   CCFAILED,=CL1'Y'
         BE    0(R15)                  so exit
*
* do cross check: VCVDQ    - VECTOR CONVERT TO DECIMAL (128)
*
         VCVBQ x'00'
         VST   V3,XCOUTPUT
...
XCFAILMSG DS   0H
         ...
         MVC   PRT3,EDIT
         ED    PRT3,DECNUM
         MVC   XCPI3(3),PRT3+13     fill in message with i3 field
*
...
CCPRTLINE DC   C'      Test # '
CCPRTNUM DC    C'xxx'
         DC    C' failed: wrong CC     for instruction '
...
CCFAILED DS    X           CC  check failed

WK1      DS    F
         DS    0F
**
*        test routine will be here (from VRI-j macro)
                                                                SPACE 2
ZVE6TST  CSECT ,
         DS    0F
                                                                SPACE 2
...
*
         MACRO
         VRI_J &INST,&I3,&M4,&CC,&SKIP
.*                               &INST  - VRI-j instruction under test
.*                               &i3    - i3 field
.*                               &m4    - m4 field
.*                               &CC    - expected CC
...
XCO&TNUM DS    XL16              Cross check Output
         DS    FD                   gap
         DS    2F          extract PSW after test (has CC)
         DS    X           extracted cc
         DS    X           CC Check failed
.*
*
X&TNUM   DS    0F
         VL    V2,V1FUDGE
         ...
         &INST &I3,&M4            test instruction (dest is source)
...
*
*----------------------------------------------------------------------
*   VRI-j   instruction, i3, m4, CC, SKIP     (Skip cross check if "S")
*   followed by
*              v1 - 16 byte expected result - packed-decimal
*              v2 - 16 byte source vector   - 128-bit binary
*----------------------------------------------------------------------
*  VCVDQ    - VECTOR CONVERT TO DECIMAL (128)
*----------------------------------------------------------------------
*        i3:   x'9f"   (IOM=1, RDC=31)
*        m4:   x'1'    (LB=0, P1=0, CS=1)
*
         VRI_J VCVDQ,X'9F',X'1',0,N
         DC    XL16'0000000000000000 000000000000000C'      v1
         DC    XL16'0000000000000000 0000000000000000'      v2

         VRI_J VCVDQ,X'9F',X'1',0,N
         DC    XL16'0000000000000000 000000000000001C'      v1
         DC    XL16'0000000000000000 0000000000000001'      v2

         VRI_J VCVDQ,X'9F',X'1',0,N
         DC    XL16'0000000000000000 000000000000001D'      v1
         DC    XL16'FFFFFFFFFFFFFFFF FFFFFFFFFFFFFFFF'      v2

         VRI_J VCVDQ,X'9F',X'1',0,N
         DC    XL16'0000000000009223 372036854775807C'      v1
         DC    XL16'0000000000000000 7FFFFFFFFFFFFFFF'      v2

         VRI_J VCVDQ,X'9F',X'1',0,N
         DC    XL16'0000000000009223 372036854775807D'      v1
         DC    XL16'FFFFFFFFFFFFFFFF 8000000000000001'      v2

         VRI_J VCVDQ,X'9F',X'1',0,N
         DC    XL16'9999999999999999 999999999999999C'      v1
         DC    XL16'0000007E37BE2022 C0914B267FFFFFFF'      v2

         VRI_J VCVDQ,X'9F',X'1',0,N
         DC    XL16'9999999999999999 999999999999999D'      v1
         DC    XL16'FFFFFF81C841DFDD 3F6EB4D980000001'      v2
*
* overflow
*
         VRI_J VCVDQ,X'9F',X'1',3,N
         DC    XL16'0000000000000000 000000000000000C'      v1
         DC    XL16'0000007E37BE2022 C0914B2680000000'      v2

         VRI_J VCVDQ,X'9F',X'1',3,N
         DC    XL16'0000000000000000 000000000000000C'      v1
         DC    XL16'FFFFFF81C841DFDD 3F6EB4D980000000'      v2

*----------------------------------------------------------------------
*        i3:   x'9f"   (IOM=1, RDC=31)
*        m4:   x'3'    (LB=0, P1=1, CS=1)
*
         VRI_J VCVDQ,X'9F',X'3',0,N
         DC    XL16'0000000000000000 000000000000000F'      v1
         DC    XL16'0000000000000000 0000000000000000'      v2

         VRI_J VCVDQ,X'9F',X'3',0,N
         DC    XL16'0000000000000000 000000000000001F'      v1
         DC    XL16'0000000000000000 0000000000000001'      v2

         VRI_J VCVDQ,X'9F',X'3',0,S
         DC    XL16'0000000000000000 000000000000001F'      v1
         DC    XL16'FFFFFFFFFFFFFFFF FFFFFFFFFFFFFFFF'      v2

         VRI_J VCVDQ,X'9F',X'3',0,N
         DC    XL16'0000000000009223 372036854775807F'      v1
         DC    XL16'0000000000000000 7FFFFFFFFFFFFFFF'      v2
         ...
         VRI_J VCVDQ,X'9F',X'3',0,N
         DC    XL16'9999999999999999 999999999999999F'      v1
         DC    XL16'0000007E37BE2022 C0914B267FFFFFFF'      v2
...
*----------------------------------------------------------------------
*        i3:   x'9f"   (IOM=1, RDC=31)
*        m4:   x'9'    (LB=1, P1=0, CS=1)
*
         VRI_J VCVDQ,X'9F',X'9',0,N
         DC    XL16'0000000000000000 000000000000000C'      v1
         DC    XL16'0000000000000000 0000000000000000'      v2
         ...
         VRI_J VCVDQ,X'9F',X'9',3,S
         DC    XL16'6920938463463374 607431768211455C'      v1
         DC    XL16'FFFFFFFFFFFFFFFF FFFFFFFFFFFFFFFF'      v2
         ...
         VRI_J VCVDQ,X'9F',X'9',3,S
         DC    XL16'6920938463454151 235394913435649C'      v1
         DC    XL16'FFFFFFFFFFFFFFFF 8000000000000001'      v2
         ...
         VRI_J VCVDQ,X'9F',X'9',3,S
         DC    XL16'6920938463463374 607431768211457C'      v1
         DC    XL16'FFFFFF81C841DFDD 3F6EB4D980000001'      v2
*
* overflow
*
         VRI_J VCVDQ,X'9F',X'9',3,N
         DC    XL16'0000000000000000 000000000000000C'      v1
         DC    XL16'0000007E37BE2022 C0914B2680000000'      v2

         VRI_J VCVDQ,X'9F',X'9',3,N
         DC    XL16'6920938463463374 607431768211456C'      v1
         DC    XL16'FFFFFF81C841DFDD 3F6EB4D980000000'      v2


*----------------------------------------------------------------------
*        i3:   x'9f"   (IOM=1, RDC=31)
*        m4:   x'B'    (LB=1, P1=1, CS=1)
*
         VRI_J VCVDQ,X'9F',X'B',0,N
         DC    XL16'0000000000000000 000000000000000F'      v1
         DC    XL16'0000000000000000 0000000000000000'      v2
         ...
         VRI_J VCVDQ,X'9F',X'B',3,S
         DC    XL16'6920938463463374 607431768211455F'      v1
         DC    XL16'FFFFFFFFFFFFFFFF FFFFFFFFFFFFFFFF'      v2
         ...
         VRI_J VCVDQ,X'9F',X'B',3,S
         DC    XL16'6920938463454151 235394913435649F'      v1
         DC    XL16'FFFFFFFFFFFFFFFF 8000000000000001'      v2
         ...
         VRI_J VCVDQ,X'9F',X'B',3,S
         DC    XL16'6920938463463374 607431768211457F'      v1
         DC    XL16'FFFFFF81C841DFDD 3F6EB4D980000001'      v2
*
* overflow
*
         VRI_J VCVDQ,X'9F',X'B',3,N
         DC    XL16'0000000000000000 000000000000000F'      v1
         DC    XL16'0000007E37BE2022 C0914B2680000000'      v2

         VRI_J VCVDQ,X'9F',X'B',3,N
         DC    XL16'6920938463463374 607431768211456F'      v1
         DC    XL16'FFFFFF81C841DFDD 3F6EB4D980000000'      v2

*----------------------------------------------------------------------
*  VCVDQ    - VECTOR CONVERT TO DECIMAL (128)
*----------------------------------------------------------------------
*        i3:   x'8f"   (IOM=1, RDC=15)
*        m4:   x'1'    (LB=0, P1=0, CS=1)
*
         VRI_J VCVDQ,X'8F',X'1',0,N
         DC    XL16'0000000000000000 000000000000000C'      v1
         DC    XL16'0000000000000000 0000000000000000'      v2
...
* overflow
*
         VRI_J VCVDQ,X'8F',X'1',3,N
         DC    XL16'0000000000000000 372036854775807C'      v1
         DC    XL16'0000000000000000 7FFFFFFFFFFFFFFF'      v2

         VRI_J VCVDQ,X'8F',X'1',3,N
         DC    XL16'0000000000000000 372036854775807D'      v1
         DC    XL16'FFFFFFFFFFFFFFFF 8000000000000001'      v2

         VRI_J VCVDQ,X'8F',X'1',3,N
         DC    XL16'0000000000000000 999999999999999C'      v1
         DC    XL16'0000007E37BE2022 C0914B267FFFFFFF'      v2

         VRI_J VCVDQ,X'8F',X'1',3,N
         DC    XL16'0000000000000000 999999999999999D'      v1
         DC    XL16'FFFFFF81C841DFDD 3F6EB4D980000001'      v2
...
*
         ...
         VRI_J VCVDQ,X'8F',X'3',0,S
         DC    XL16'0000000000000000 000000000000001F'      v1
         DC    XL16'FFFFFFFFFFFFFFFF FFFFFFFFFFFFFFFF'      v2
*
* overflow
*
         VRI_J VCVDQ,X'8F',X'3',3,N
         DC    XL16'0000000000000000 372036854775807F'      v1
         DC    XL16'0000000000000000 7FFFFFFFFFFFFFFF'      v2

         VRI_J VCVDQ,X'8F',X'3',3,S
         DC    XL16'0000000000000000 372036854775807F'      v1
         DC    XL16'FFFFFFFFFFFFFFFF 8000000000000001'      v2

         VRI_J VCVDQ,X'8F',X'3',3,N
         DC    XL16'0000000000000000 999999999999999F'      v1
         DC    XL16'0000007E37BE2022 C0914B267FFFFFFF'      v2
...
* overflow
*
         VRI_J VCVDQ,X'8F',X'9',3,S
         DC    XL16'0000000000000000 607431768211455C'      v1
         DC    XL16'FFFFFFFFFFFFFFFF FFFFFFFFFFFFFFFF'      v2

         VRI_J VCVDQ,X'8F',X'9',3,N
         DC    XL16'0000000000000000 372036854775807C'      v1
         DC    XL16'0000000000000000 7FFFFFFFFFFFFFFF'      v2

         VRI_J VCVDQ,X'8F',X'9',3,S
         DC    XL16'0000000000000000 235394913435649C'      v1
         DC    XL16'FFFFFFFFFFFFFFFF 8000000000000001'      v2

         VRI_J VCVDQ,X'8F',X'9',3,N
         DC    XL16'0000000000000000999999999999999C'      v1
         DC    XL16'0000007E37BE2022 C0914B267FFFFFFF'      v2

         VRI_J VCVDQ,X'8F',X'9',3,S
         DC    XL16'0000000000000000 607431768211457C'      v1
         DC    XL16'FFFFFF81C841DFDD 3F6EB4D980000001'      v2
         ...
         VRI_J VCVDQ,X'8F',X'9',3,N
         DC    XL16'0000000000000000 607431768211456C'      v1
         DC    XL16'FFFFFF81C841DFDD 3F6EB4D980000000'      v2
...
* overflow
*
         VRI_J VCVDQ,X'8F',X'B',3,S
         DC    XL16'0000000000000000 607431768211455F'      v1
         DC    XL16'FFFFFFFFFFFFFFFF FFFFFFFFFFFFFFFF'      v2

         VRI_J VCVDQ,X'8F',X'B',3,N
         DC    XL16'0000000000000000 372036854775807F'      v1
         DC    XL16'0000000000000000 7FFFFFFFFFFFFFFF'      v2

         VRI_J VCVDQ,X'8F',X'B',3,S
         DC    XL16'0000000000000000 235394913435649F'      v1
         DC    XL16'FFFFFFFFFFFFFFFF 8000000000000001'      v2
         ...
         VRI_J VCVDQ,X'8F',X'B',3,S
         DC    XL16'0000000000000000 607431768211457F'      v1
         DC    XL16'FFFFFF81C841DFDD 3F6EB4D980000001'      v2
         ...
         VRI_J VCVDQ,X'8F',X'B',3,N
         DC    XL16'0000000000000000 607431768211456F'      v1
         DC    XL16'FFFFFF81C841DFDD 3F6EB4D980000000'      v2
