...
/*                                                                   */
/*   dasdinit64 [-options] filename devtype[-model] [volser] [size]  */
/*                                                                   */
/* options    options:                                               */
...
#define UTILITY_NAME    "dasdinit64"
#define UTILITY_DESC    "64-bit DASD image file creation program"

static void argexit( int code, char* m, const char* pgm );

/*-------------------------------------------------------------------*/
/* DASDINIT64 program main entry point                               */
/*-------------------------------------------------------------------*/
...
int     rc;                             /* Return code               */
    ...
    if (type == 'C')
        rc = create_ckd64( fname, devtype, heads, maxdlen, size, volser,
                           comp, (BYTE) lfs, (BYTE) 0, (BYTE) nullfmt,
                           (BYTE) rawflag, (BYTE) flagECmode,
                           (BYTE) flagMachinecheck );
    else
        rc = create_fba64( fname, devtype, sectsize, size, volser, comp,
                           lfs, 0, rawflag );
