/* CCKDDIAG64.C (C) Copyright Roger Bowler, 1999-2012                */
/*              Hercules CCKD64 diagnostic tool                      */
/*                                                                   */
...
#define UTILITY_NAME    "cckddiag64"
#define UTILITY_DESC    "CCKD64/CFBA64 diagnostic program"
...
/* Global data areas                                                 */
/*-------------------------------------------------------------------*/
static CCKD64_L1ENT   *L1tab     = NULL;    /* Level 1 table pointer */
static CCKD64_L2ENT   *L2tab     = NULL;    /* Level 2 table pointer */
...
(
    ...
    U64           offset,       /* read offset into CCKD image       */
...
)
{
    ...
    unsigned int    ubufl;      /* when U64 != unsigned int          */
#endif
...
/*-------------------------------------------------------------------*/
/* offtify - given decimal or hex input string, return off_t as U64  */
/*-------------------------------------------------------------------*/
...
U64             op_offset    = 0;       /* op_offset of length       */
...
U64             L2taboff     = 0;       /* offset to assoc. L2 table */
...
char            pathname[ MAX_PATH ];   /* file path in host format  */
    ...
    /*---------------------------------------------------------------*/
    if (is_dh_devid_typ( devhdr.dh_devid, CKD64_CMP_OR_SF_TYP ))
    {
        ckddasd = true;
    ...
    else if (is_dh_devid_typ( devhdr.dh_devid, FBA64_CMP_OR_SF_TYP ))
    {
        ckddasd = false;
    ...
    L1tab = makbuf(                       num_L1tab * CCKD64_L1ENT_SIZE, "L1TAB64" );
    readpos( fd, L1tab, CCKD64_L1TAB_POS, num_L1tab * CCKD64_L1ENT_SIZE );
    ...
    if (cmd_l1tab)
    {
        // "%s - %d (decimal) bytes:"
        printf("\n");
        WRMSG( HHC02614, "I", "L1TAB", (int) (num_L1tab * CCKD64_L1ENT_SIZE) );
        data_dump( L1tab, num_L1tab * CCKD64_L1ENT_SIZE );
        printf("\n");
    }
    ...
    if ((cmd_cchh) || (cmd_tt))
    {
        ...
        if (cmd_l2tab)
        {
            ...
            WRMSG( HHC02614, "I", "L2TAB",
                (int)         (cdevhdr.num_L2tab * CCKD64_L2ENT_SIZE));
            data_dump( L2tab, (cdevhdr.num_L2tab * CCKD64_L2ENT_SIZE));
        }

        // "L2 index %d = L2TAB entry: %d bytes:"
        printf("\n");
        WRMSG( HHC02611, "I", L2ndx, (int) CCKD64_L2ENT_SIZE);
        data_dump( &L2tab[ L2ndx ],        CCKD64_L2ENT_SIZE);
        printf("\n");
