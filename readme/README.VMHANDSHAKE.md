...
1. X'F1' - Spool file open (specifies the spool file name)
2. X'FF' - Spool file close (SLI bit recommended)
3. X'F9' - Spool file close and name
...
* The *name* of the file will be the value specified on the X'D9' CCW. Any sequence of spaces is reduced
to a single space and all spaces are converted to underscores.
...
the behavior of the printer and card punch device handlers is unchanged except for the changed `append` semantics. This option is not allowed
