...
Because multivolume support is not necessarily VOL1/HDR1/EOV/EOF based, a certain number of new features have to be implemented in order to let the guest program manage the multivolume on it's own. (ex: VM/DDR, DOS Tape Spooled output, etc..)
...
<b>strictsize=0|1</b>:
        ...
        the file will be truncated on the preceding tape block boundary.
...
However, if the sense is not the result of a previously occurring Unit Check, a new sense is built to reflect the current device status.
Also, this management is a necessary step in order to eventually implement multipath operations (Contingency Allegiance status).
