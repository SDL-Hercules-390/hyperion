...
One of the adjustments that must be made is to either point your **`LIBRARY_PATH`** and **`CPATH`** variables to your `./extpkgs/lib` and `./extpkgs/include` directories or else specify the top level `./extpkgs` directory for your `--enable-extpkgs=DIR` configure option.
