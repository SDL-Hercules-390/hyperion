...
RADR    n;                              /* Working abs page address  */
#if defined( _FEATURE_SIE )
BYTE    storkey;
#endif
    ...
    if (SIE_MODE( regs ))
    {
        ...
        if (!regs->sie_pref)
        {
            ...
            if (SIE_STATE_BIT_ON( regs, RCPO0, ASIST )
...
BYTE    storkey;                        /* Original Storage key      */
...
VADR    effective_addr1;                /* Effective address         */
    ...
    if ((effective_addr1 == 0x00000950) && (i2 == 0xff))
        if (!(ARCH_DEP( vfetchb )( 1376 + ARCH_DEP( vfetch4 )( 2892, 0, regs ), 0, regs ) & 0x80))
